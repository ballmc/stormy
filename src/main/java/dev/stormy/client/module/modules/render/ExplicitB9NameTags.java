...
import dev.stormy.client.utils.player.CombatUtils;
...
public class ExplicitB9NameTags extends Module {
    ...
    public ExplicitB9NameTags() {
        super("(ExplicitB9)NameTags", ModuleCategory.Render, 0);
    ...
    @SubscribeEvent
    public void nameTag(RenderLabelEvent renderLivingEvent) {
        if (renderLivingEvent.getTarget() instanceof EntityPlayer) {
            EntityPlayer player = (EntityPlayer) renderLivingEvent.getTarget();
            ...
            boolean canTarget = CombatUtils.canTarget(player, true);
            boolean withinRange = ((double) Minecraft.getMinecraft().thePlayer.getDistanceToEntity(player) <= rangeSetting.getInput() || rangeSetting.getInput() == 0.0D);
            if (nameNotNull && nameNotEmpty && canTarget && withinRange) {
                // Don't do this
                renderLivingEvent.setCancelled(true);
            }
        }
    }

    @SubscribeEvent
    public void render3d(RenderWorldEvent renderWorldLastEvent) {
        ...
        while (playerIterator.hasNext()) {
            ...
            if (CombatUtils.canTarget(player, true)) {
                ...
                _x = (float) (player.lastTickPosX + (player.posX - player.lastTickPosX) * (double) Utils.getTimer().renderPartialTicks - Minecraft.getMinecraft().getRenderManager().viewerPosX);
                _y = (float) (player.lastTickPosY + (player.posY - player.lastTickPosY) * (double) Utils.getTimer().renderPartialTicks - Minecraft.getMinecraft().getRenderManager().viewerPosY);
                _z = (float) (player.lastTickPosZ + (player.posZ - player.lastTickPosZ) * (double) Utils.getTimer().renderPartialTicks - Minecraft.getMinecraft().getRenderManager().viewerPosZ);
